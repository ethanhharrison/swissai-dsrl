#!/bin/bash
# Submit multi-task DSRL runs on libero_90 tasks {28, 79, 44, 59, 43}.
#
# Submit:  bash sbatch_multitask_28-79-44-59-43_all.sh
set -euo pipefail
...
jobid0=$(sbatch -A co_rail -p savio4_gpu --gres=gpu:A5000:1 -N 1 -n 1 -c 4 --qos=rail_gpu4_high -t 72:00:00 --mem=60G \
  -J dsrl_multi28-79-44-59-43_utd20_s0 \
  -o "${LOGDIR}/dsrl_multi28-79-44-59-43_utd20_s0.%j.out" \
  -e "${LOGDIR}/dsrl_multi28-79-44-59-43_utd20_s0.%j.err" \
  --requeue --parsable \
  --wrap "${CONDA_SETUP} && ${RUN_CMD} 0 --wandb_project DSRL_pi0_Libero_MultiTask") \
  && echo "submitted $jobid0  (dsrl_multi28-79-44-59-43_utd20_s0)"

jobid1=$(sbatch -A co_rail -p savio4_gpu --gres=gpu:A5000:1 -N 1 -n 1 -c 4 --qos=rail_gpu4_high -t 72:00:00 --mem=60G \
  -J dsrl_multi28-79-44-59-43_utd20_s1 \
  -o "${LOGDIR}/dsrl_multi28-79-44-59-43_utd20_s1.%j.out" \
  -e "${LOGDIR}/dsrl_multi28-79-44-59-43_utd20_s1.%j.err" \
  --requeue --parsable \
  --wrap "${CONDA_SETUP} && ${RUN_CMD} 1 --wandb_project DSRL_pi0_Libero_MultiTask") \
  && echo "submitted $jobid1  (dsrl_multi28-79-44-59-43_utd20_s1)"
